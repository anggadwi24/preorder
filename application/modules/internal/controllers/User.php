...
class User extends MX_Controller 
{
	...
	function status(){
		...
		if($cek->num_rows() > 0){
			...
			$this->model_app->update('users',array('users_status'=>$status),array('users_id'=>$id));
	...
	function store(){
		if($this->input->method() == 'post'){
			...
			$this->form_validation->set_rules('notelp','No telepon','min_length[5]|max_length[20]|required');
			...
			if($this->form_validation->run() == FALSE){
				...
				if ($this->upload->do_upload('gambar')){
					...
					$data = array(
						...
						'users_no_telp' => $this->input->post('notelp'),
