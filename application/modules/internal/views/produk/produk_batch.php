                    ...
                    <?php 
                        if($record->num_rows() > 0){
                            ...
                            foreach($record->result_array() as $rows){
                                ...
                                $buyer = $this->db->query("SELECT coalesce(SUM(td_qty),0) as total FROM transaksi a JOIN transaksi_detail b ON a.transaksi_id = b.td_transaksi_id WHERE td_produk_id = ".$row['produk_id']."  AND td_pb_id = ".$rows['pb_id']." AND (transaksi_status = 'dibayar' OR transaksi_status ='selesai' )")->row_array();
                                echo '<tr>
                                <td>
